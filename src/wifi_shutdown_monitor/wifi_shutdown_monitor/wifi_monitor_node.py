    ...
    def check_wifi_connection(self) -> None:
        if self.is_connected_to_ssid():
            ...
            self.get_logger().warn(
                f"Not connected to {self.ssid}. Time since last connection: {time_since_last_connection:.2f} seconds"
            )

            if time_since_last_connection > self.timeout:
                self.get_logger().error(
                    f"WiFi connection to {self.ssid} lost for more than {self.timeout} seconds. Shutting down..."
                )
                self.shutdown_robot()
